...
members, and mobile money industry partners to support the market
interoperability using the Mojaloop technology. Thus, the Inclusive Tech Lab has
dedicated efforts to create a platform for interoperability testing.

This test environment will provide a set of simulators to test different
scenarios, where the System Under Test (SUT) can be a Service Provider or a
Mobile Money Operator. The test environment will be composed of a portal, test
control and five simulators, each of which can be replaced with an operational
software. The diagram below describes an example of this flow.
...
components in the diagram above to assess its behaviour in different scenarios. This
allows the interoperability of the system to be tested through several test
cases, evaluating the different paths that transactions can take. This project
is still on the rise and our goal is to add more components and assessment
scenarios. Also, it uses the MIT license, which allows interested
parties to use the platform and its source code.
