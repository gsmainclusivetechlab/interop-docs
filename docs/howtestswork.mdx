...
There are five main components used to test the paths corresponding to different
use cases. They are:

- **SP Simulator**: Acts as a mock service provider (SP) such as a utilities
  company or supermarket checkout to simulate their role in a transaction.
- **MMO1 simulator**: Acts as a mock financial services provider (FSP)
  representing the _payee_ of a transaction.
- **MMO1 simulator**: Acts as a mock financial services provider (FSP)
  representing the _payer_ of a transaction.
- **Mojaloop Simulator**: Acts as an interoperability platform that connects
  MMO1 and MMO2 in a transaction.
- **Interoperability Test Platform**: The core test platform which intercepts
