---
id: fsp
title: Financial Services Provider Installation Guide
sidebar_label: Financial Services Providers
---

The GSMA Interoperability Test Platform currently supports simulation of
Financial Services Providers (FSP) in two configurations. In the first
configuration, the FSP can communicate with a service provider via the
[GSMA Mobile Money API](https://developer.mobilemoneyapi.io/) and with an
interoperability hub using the Mojaloop API (FSP1 in the diagram below). In the
second configuration, the FSP will communicate exclusively with the Mojaloop API
(FSP2 in the diagram below).
...
<Graphviz
  ...
  dot={`
    digraph { 
      ...
      FSP2 [label="FSP2"]
      sp -> FSP1 [dir="both", color="#DE002B", label="MM API", fontcolor="#DE002B"]
      FSP1 -> mojaloop -> FSP2 [dir="both", label="Mojaloop API", fontcolor="#00A3FF", color="#00A3FF"]
    }
`}
/>

## FSP1 Simulator Set-up
...
- Within FSP1, `my_uuid` should always be equal to
...
- Within FSP2, `my_uuid` should always be equal to
  `e602a859-a25f-4d37-9abe-0ac09fb734af`
- Within FSP2, `recipient_uuid` should always be equal to
...
database with the following command (this is not necessary on FSP2, where no
database is used):
...
Once you have obtained the latest code, the procedure differs slightly for FSP1
and FSP2.

#### FSP1

For FSP1, the services can immediately be restarted, and any missing
...
For FSP2, the simulator image must be rebuilt before restarting the services,
