...
validation and Business Rule validations. Business rule validations are
controlled by custom logic defined within the test case specification, while
schema validation is powered by the
...
to Financial Services Provider 1.

### Financial Services Provider 1

Acts as a mock financial services provider (FSP) such as a Mobile Money Operator
or Bank to simulate their role in a transaction. FSP 1 will always act as the
initiator of a transaction.<br /> This simulator can perform two types of
actions:

- Receive messages through the GSMA Mobile Money API and map them to a Mojaloop
  API.<br /> This simulates a scenario when a Financial Services Provider
  receives a request from a Service Provider to perform a transaction, acting as
  the payee. For more details, see the
  [Merchant-Initiated Merchant Payment](/usecases/list#mimp) use case.
- Initiate messages directly through Mojaloop API.<br /> This simulates a
  ...
  payer. For more details, see the [P2P](/usecases/list#p2p) use case.

### Financial Services Provider 2

Acts as a mock financial services provider (FSP) such as a Mobile Money Operator
or Bank to simulate their role in a transaction. While FSP1 always acts as the
_initiator_ of a transaction, FSP2 always acts as the recipient of a
transaction. Note that the recipient of a transaction may be the sender of
funds, as in the case of a merchant-initiated transaction request.<br /> This
simulator can perform one action:
...
- Send and receive messages through the Mojaloop API.<br /> This simulates a
  ...
  details, see the [Merchant-Initiated Merchant Payment](/usecases/list#mimp)
  use case. The same flow simulates a scenario when an FSP replies to a
  transaction from a Mojaloop network, acting as payee. For more details, see
  the [P2P](/usecases/list#p2p) use case.
...
Financial Services Providers.<br /> Once a SUT is selected in the platform, it
