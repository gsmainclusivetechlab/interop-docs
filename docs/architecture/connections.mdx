...
In normal operation, the payer's FSP would connect directly to Mojaloop to
arrange a funds transfer to the payee's FSP. In testing, however, we must
  ...
  dot={`
    digraph {
      ...
      subgraph sim {
        ...
        sp -> FSP1 [color="#67ab9f"]
        FSP1 -> mojaloop -> FSP2 
      }
      {
        edge[dir="both"]
        ITP -> {sp FSP1} [color="#67ab9f"]
        ITP -> {FSP1 mojaloop FSP2}
...
- **FSP1**: e5f5e817-94d6-4a43-a7ec-f7274b6d85c2
- **Mojaloop**: b2a85076-b748-4d93-8df1-2b39844e6d4b
- **FSP2**: e602a859-a25f-4d37-9abe-0ac09fb734af
...
Thus, the URL that an FSP1 test subject should use to send messages to Mojaloop
...
without the `session_uuid`.

```handlebars
https://{itp_url}/testing/{my_uuid}/{recipient_uuid}/simulator/
```

Thus, the URL that an FSP1 simulator should use to send messages to Mojaloop
(through the test platform) would be:
